...
# def write_mincobj(coords, faces, attributes, filename):
#     sys.stdout.write('Writing m file ' + filename + '...')
#     vertices_siz = coords.shape[0] if coords.shape[0] > coords.shape[1] else coords.shape[1]
#     fid = open(filename, 'wt')
#     fid.write('P 0.3 0.7 0.5 100 1 ')
#     fid.write('{0:d}\n'.format(vertices_siz))
#
#     for i in np.arange(0, vertices_siz):
#         fid.write("Vertex {0:d} {1:f} {2:f} {3:f}\n".format(int(i+1), float(coords[i,0]),float(coords[i,1]),float(coords[i,2])))
#


def writeccbbm(coords, faces, attributes, filename):

    sys.stdout.write('Writing m file ' + filename + '...')
    vertices_siz = coords.shape[0] if coords.shape[0] > coords.shape[1] else coords.shape[1]
    fid = open(filename, 'wt')
    fid.write('# CCBBM 1.0\n')
    fid.write('# Generated by surfio on Tue Jan 10 10:35:04 PST 2012\n')

    for i in np.arange(0, vertices_siz):
        fid.write("Vertex {0:d} {1:f} {2:f} {3:f}\n".format(int(i), float(coords[i,0]),float(coords[i,1]),float(coords[i,2])))

    faces_siz = faces.shape[0] if faces.shape[0] > faces.shape[1] else faces.shape[1]
    for i in np.arange(0, faces_siz):
        fid.write("Face {0:d} {1:d} {2:d} {3:d}\n".format(int(i), int(faces[i,0]), int(faces[i,1]), int(faces[i,2])))
...
def writesurface_new(filename,coords,faces,attributes=[],isMultilevelUCF=False):
    ...
    def ccbbm(filename):
        writeccbbm(coords, faces, attributes, filename)
